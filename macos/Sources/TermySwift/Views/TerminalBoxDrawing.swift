...
enum TerminalBoxDrawing {
    ...
    static func segments(for character: Character) -> TerminalBoxSegments? {
        ...
        // renderer.
        switch scalar {
        case 0x2500, 0x2504, 0x2508, 0x254C: return arms(.none, .none, .light, .light)
        case 0x2501, 0x2505, 0x2509, 0x254D: return arms(.none, .none, .heavy, .heavy)
        case 0x2502, 0x2506, 0x250A, 0x254E: return arms(.light, .light, .none, .none)
        case 0x2503, 0x2507, 0x250B, 0x254F: return arms(.heavy, .heavy, .none, .none)
    ...
    /// Converts a segment descriptor into cell-relative rects using Ghostty's
    ...
    static func geometry(
        ...
        let upBottom: CGFloat
        if segments.left.isHeavy || segments.right.isHeavy {
            upBottom = hHeavyBottom
        } else if segments.left != segments.right || segments.down == segments.up {
            upBottom = (segments.left.isDouble || segments.right.isDouble)
        ...
        if segments.left.isHeavy || segments.right.isHeavy {
            ...
            downTop = (segments.left.isDouble || segments.right.isDouble)
        ...
        if segments.up.isHeavy || segments.down.isHeavy {
            leftRight = vHeavyRight
        } else if segments.up != segments.down || segments.left == segments.right {
            leftRight = (segments.up.isDouble || segments.down.isDouble)
