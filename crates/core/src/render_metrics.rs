...
pub fn increment_grid_paint_count() {
    increment_counter(&GRID_PAINT_COUNT);
}

pub fn increment_shape_line_calls() {
    increment_counter(&SHAPE_LINE_CALLS);
}

pub fn increment_shaped_line_cache_hit() {
    increment_counter(&SHAPED_LINE_CACHE_HITS);
}

pub fn increment_shaped_line_cache_miss() {
...
pub fn add_span_row_ops_rebuild_us(micros: u64) {
    add_to_counter(&SPAN_ROW_OPS_REBUILD_US, micros);
}

pub fn add_span_text_shaping_us(micros: u64) {
    add_to_counter(&SPAN_TEXT_SHAPING_US, micros);
}

pub fn add_span_grid_paint_us(micros: u64) {
