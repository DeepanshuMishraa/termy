...
Use [tasks](/docs/using-termy/tasks-layouts) with the `layout` field to scope a task to a saved layout. The tmux session is restored as part of that layout, so a task like:
...
- [Terminal Behavior settings](/docs/customize/terminal-behavior)
